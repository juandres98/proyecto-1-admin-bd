...
myConnection = psycopg2.connect(host = 'localhost', user= 'postgres', password ='administracion', dbname= 'sambil')
ListaPersonaN= []
ListaPersonaS = []
...
def recorridoNo(persona):
    # numero = round(randint(0,1))#random para elegir entre una persona con telefono y una sin telefono
    # # if(numero==0):
    # tam=len(ListaPersonaN)-1
    # elegido = round(randint(0,tam))
    # persona = ListaPersonaN[elegido]
...
def feria(persona):
    ...
    if select==1:
        ...
        cur.close()
        cur = myConnection.cursor()
        print("sirve hasta aqui")
        # cur.execute("""INSERT INTO ocupacion(idpersona, estado, idmesa) values(%s,1,1); """,persona)
...
def comprarno(id):
    ...
    cur = myConnection.cursor()
    # cur.execute("INSERT INTO comprano( idtienda, idpersona, monto) VALUES( %s, %s, %s);", ( idtienda, id, monto))
...
def comprarSi(id):
    ...
    #Random entre 5000 y un millon
    cur = myConnection.cursor()
    # cur.execute("INSERT INTO compratl( idtienda, idpersona, monto) VALUES( %s, %s, %s);", ( idtienda, id, monto))
